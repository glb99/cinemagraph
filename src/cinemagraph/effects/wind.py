...
def _precompute(ctx: EffectContext) -> dict:
    ...
    return {
        "yy": yy, "xx": xx, "mask": ctx.mask, "amplitude": amplitude, "h": ctx.h,
        "spatial_freqs": spatial_freqs, "weights": weights, "cycles": cycles, "phases": phases,
    }


def _apply(base, pc, t):
    disp = np.zeros(pc["yy"].shape, dtype=np.float32)
    for freq, wgt, cyc, ph in zip(pc["spatial_freqs"], pc["weights"], pc["cycles"], pc["phases"]):
        disp += wgt * np.sin(2 * np.pi * cyc * t + pc["yy"] * freq / pc["h"] * 2 * np.pi + ph)
...
WIND = register(Effect(
    name="wind", family=EffectFamily.TONE,
    precompute=_precompute, apply=_apply,
    defaults=DEFAULTS, allowed_kwargs=frozenset({"amplitude", "gustiness"}),
))
