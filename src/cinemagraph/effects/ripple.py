...
def _precompute(ctx: EffectContext) -> dict:
    ...
    cycles = cycles_for(speed_hz=0.5, duration=ctx.duration, speed=ctx.speed)  # ~2 cycles at 4s default
...
RIPPLE = register(Effect(
    name="ripple", family=EffectFamily.TONE,
    precompute=_precompute, apply=_apply,
    defaults=DEFAULTS, allowed_kwargs=frozenset({"amplitude", "wavelength"}),
))
