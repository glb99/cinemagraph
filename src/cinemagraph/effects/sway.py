...
def _precompute(ctx: EffectContext) -> dict:
    ...
    cycles = cycles_for(speed_hz=0.25, duration=ctx.duration, speed=ctx.speed)  # ~1 cycle at 4s default
    yy, xx = np.meshgrid(np.arange(ctx.h), np.arange(ctx.w), indexing="ij")
    return {"cycles": cycles, "yy": yy, "xx": xx, "mask": ctx.mask,
            "amplitude": amplitude, "freq": freq, "h": ctx.h}
...
SWAY = register(Effect(
    name="sway", family=EffectFamily.TONE,
    precompute=_precompute, apply=_apply,
    defaults=DEFAULTS, allowed_kwargs=frozenset({"amplitude", "freq"}),
))
