...
def _particles_precompute_raw(image, duration, count, min_speed, max_speed, size_range,
...
def _make_particle_effect(name: str) -> Effect:
    ...
    def _precompute(ctx: EffectContext) -> dict:
        ...
        pc = _particles_precompute_raw(ctx.image, ctx.duration, seed=ctx.kwargs.get("seed", 0),
                                        speed=ctx.speed, **preset)
        return {"pc": pc, "opacity": opacity, "streak_ksize": streak_ksize,
    ...
    return Effect(
        name=name, family=EffectFamily.PARTICLE,
        precompute=_precompute, apply=_apply,
        defaults={k: v for k, v in preset_template.items() if k in ("count", "opacity")},
