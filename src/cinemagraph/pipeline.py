...
from . import effects, io_utils
from . import grade as grade_mod
from . import loop as loop_mod
from . import mask as mask_mod

_LOOP_DURATION_GIF_ERROR = "loop_duration and also_gif can't be used together (the GIF would be enormous); drop one of them."
...
def render_video_cinemagraph(
    ...
    if still_frame_index >= len(frames):
        raise ValueError(
            f"still_frame_index {still_frame_index} out of range (clip has {len(frames)} frames)"
        )
    ...
    if mask_path:
        ...
        soft_mask = mask_mod.auto_motion_mask(
            frames, threshold=mask_threshold, feather=feather
        )
    ...
    with click.progressbar(frames, label="Compositing") as bar:
        for frame in bar:
            blended = (
                still.astype(np.float32) * (1 - mask_3ch)
    ...
    if apply_grade:
        looped = grade_mod.apply_grade_to_frames(
            looped, strength=grade_strength, grain=grain
        )
...
def render_photo_cinemagraph(
    ...
    name for per-effect overrides, e.g. {"dust": {"count": 150}}.
    `unmasked_effects` (effect names) animate over the whole photo regardless
    of `mask_path`/`mask` -- see animate_photo() for the full contract.
    Returns (frames, fps)."""
    image = io_utils.read_image(photo_path)
    h, w = image.shape[:2]
    soft_mask = (
        mask_mod.load_mask(mask_path, (h, w), feather=feather) if mask_path else None
    )
    ...
    frames = effects.animate_photo(
        ...
        unmasked_effects=unmasked_effects,
    )

    if apply_grade:
        frames = grade_mod.apply_grade_to_frames(
            frames, strength=grade_strength, grain=grain
        )
...
def render_mask_preview(
    input_path: str, mask_threshold: int = 25, feather: int = 21
) -> np.ndarray:
...
def save_cinemagraph_from_photo(
    ...
    if also_gif:
        io_utils.write_gif(
            frames, str(Path(output_path).with_suffix(".gif")), fps=out_fps
        )


def save_mask_preview(
    input_path: str, output_path: str, mask_threshold: int = 25, feather: int = 21
) -> None:
    soft_mask = render_mask_preview(
        input_path, mask_threshold=mask_threshold, feather=feather
    )
