...
"sdxl"/"gemini" (image), "acestep"/"lyria3" (music), and "stable-audio"
(sound effect) are registered today (app.py, at import time; "gemini"/
"lyria3" only when GEMINI_API_KEY is set). Sound effect still has only one
adapter -- it went through the same "mechanism present, unconsumed" phase
music/image themselves started in; see docs/DESIGN.md sec 3.6 for why the
...
def available_music_remix_generators() -> tuple[str, ...]:
    """Subset of available_music_generators() whose adapter actually
    supports remix() (ACEStepAdapter today; Lyria3Adapter doesn't -- see its
    own remix() docstring). getattr(..., False) rather than requiring every
