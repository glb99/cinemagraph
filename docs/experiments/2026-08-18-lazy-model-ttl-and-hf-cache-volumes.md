...
- Six **VRAM tiers** (≤4/6/8/12/16/24GB+; this GPU is tier3) driving real capability limits — max duration, max batch size, which LM sizes are offered at all.
- **Auto CPU-offload below 20 GB**, on by default, with a comment recording that the old 16 GB threshold made 16 GB cards OOM. Disabled on Apple Silicon, where unified memory makes offload pointless.
- **Intra-request offload** — `offload_to_cpu` loads sub-models *one at a time within a single generation* (text encoder → DiT → VAE → LM), far finer-grained than an inter-request TTL.
- A **VRAM pre-flight check** that predicts a request's cost (`per_batch_gb × batch × duration_factor + safety_margin`) against free VRAM and returns a structured error instead of OOMing — skipped when offload is on, since then nothing is fully resident.
...
- `mallopt(M_MMAP_THRESHOLD, 128KB)` so CPU-offloaded tensor storage actually returns to the OS instead of sitting in the glibc arena — relevant to the host-RAM `Exited (137)`s above.
...
Worth trying next, in order: a **VRAM pre-flight check** (highest value, needs no architectural change — currently an SDXL request either fits or OOMs), and **`enable_model_cpu_offload()`** on the SDXL pipeline. If offload drops SDXL's resident footprint from ~7.1 GB to ~2 GB, the mutual-exclusivity problem largely dissolves and this TTL becomes a nice-to-have rather than the fix.
