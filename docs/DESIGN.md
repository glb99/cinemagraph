...
│   └── server/                # FastAPI door: app.py (routes, incl. serving frontend/dist at
│                               #   GET / and /ui/*) + service.py (workflows) +
...
`GET /` serves the web UI, with six tabs, one per capability the API exposes: **Photo**,
...
**Why a plain string in `ui.py`, not static files** (superseded 2026-08-14 — see the revisit
below; kept because the constraint it names is still real). Non-`.py` assets need explicit
...
What replaced it doesn't re-open that trap so much as step around it: `frontend/dist` is a *build
artifact*, never something `pip install` is expected to carry. The Docker image builds it in its
own stage and copies it in; a source checkout builds it with `bun run build`; anything else (an
API-only deployment, a fresh clone) gets a 503 page from the UI routes explaining how to build it,
while every other route keeps working. The failure mode is a missing bundle that says so, not a
...
**Revisited, 2026-08-14: that threshold was reached.** Seven tabs, three of them feature-gated,
an asset picker reused six times, a Web Audio waveform editor, per-tab job polling and a
save-to-library flow is exactly the "multiple views, client-side state worth managing" case the
paragraph above named. The whole page was ported into `frontend/` — React 19 + Vite + TanStack
Router/Query + Tailwind v4 + shadcn conventions, driving a `@hey-api/openapi-ts` client generated
from this app's own OpenAPI schema (regenerated by a pre-commit hook whenever `src/server/`
changes, so the client can't drift from the routes). Behaviour is a 1:1 port; the structural
changes are that `pollJob()`/`wireForm()` became one `useJobRunner` hook, and that **every app
route is namespaced under `/ui`** — the API serves its routes unprefixed, so a client-side
`/library` or `/assemble` shadows a real endpoint, which is not hypothetical: `/library` resolved
to the JSON endpoint the first time these routes were laid out flat. See
`docs/experiments/2026-08-14-ui-port-to-frontend.md`.

**Both follow-ups done the same day.** `server/ui.py` is deleted, and `GET /` now serves
`frontend/dist` — with `/ui/{path}` serving the same document so client-side routes survive a hard
reload, and `/assets/{path}` serving the hashed bundles. Three choices worth recording:

- **Routes, not `app.mount(StaticFiles(...))`.** A mount resolves its directory once at import
  time, which is precisely the "config frozen at import" pattern §7's `config.py` entry exists to
  get out of, and would make the path un-overridable in tests. Plain routes reading
  `Settings.frontend_dist_dir` keep it injected like everything else.
- **The SPA fallback is scoped to `/ui`, never the root.** A root-level catch-all would shadow
  `/library`, `/assemble`, `/jobs` and the rest of this API's unprefixed paths — the same collision
  the frontend's own `/ui` namespace exists to avoid, arriving from the other direction.
- **A missing bundle degrades, it doesn't fail.** No build, or an API-only deployment: the UI
  routes return a 503 page saying how to build it and everything else serves normally, matching how
  an unconfigured optional service behaves.

The Docker image builds the bundle in a `frontend-build` stage and copies only `dist/` into the
runtime image (bun and `node_modules` don't ship), with `CINEMAGRAPH_FRONTEND_DIR` set to match;
CI checks `GET /` and `GET /ui/library` actually return the built document, since `/health` passing
proves nothing about the UI. The `frontend` compose service stays as the hot-reload dev server.
...
| A `justfile`, not a `Makefile` | `make` isn't installed on the Windows host this project is developed on; `just` is. Shipping a Makefile would have meant shipping a file the author cannot run. It also avoids make's tab-sensitivity and `.PHONY` bookkeeping for a file that declares no build artifacts. `set windows-shell` pins bash so recipes behave the same everywhere |
| `gpu-image`/`gpu-audio` each stop the profile they conflict with, rather than documenting the conflict | the mutual exclusion between the torch satellites was already real, already measured, and already written down — and still entirely dependent on the operator remembering it at the moment they type a command. Encoding it in the recipe is the difference between a documented constraint and an enforced one. Deliberately *not* solved by a compose-level mechanism: Compose has no "these profiles are mutually exclusive" primitive, and inventing one via `depends_on` would misrepresent it as an ordering problem. It is the operator-level form of the admission control surveyed in `docs/experiments/2026-08-21-model-lifecycle-prior-art.md` |
| `cinemagraph doctor` lives in its own top-level `src/diagnostics/`, not `cinemagraph/doctor.py` | same cross-cutting test `asset_library` passed (§5.1): it knows about satellite URLs, Docker, the GPU and the frontend bundle, none of which the core photo/video package may learn about. Stdlib-only for the same reason too — it must run from a bare `uv sync` with no extras, since "the server extra isn't installed" is one of the things it reports. So `urllib` not `httpx`, and `nvidia-smi` parsed from CSV rather than `torch.cuda` (torch isn't a dependency of this project at all, and nvidia-smi reports the whole card including memory held by other processes — the number that actually matters when four satellites share one GPU) |
| `doctor` exits 0 by default and reports "not configured" as INFO, with `--strict` to opt into a non-zero exit (2026-08-21) | it's called from a `just` recipe and would otherwise fail a task chain for reporting exactly what it exists to report. Not-configured is a valid state for every optional service; only *configured-but-unreachable* is a real problem, which is the same configured-vs-available distinction `GET /capabilities` already draws for the web UI's tab gating. `--strict` exists because the opposite caller is equally real: a deploy gate or container healthcheck needs the failure to propagate. Two callers, opposite needs, one flag rather than two commands |
| `doctor`'s port probe reports two states, not three | distinguishing "closed" (refused instantly) from "filtered" (dropped, connection hangs) is genuinely useful — filtered is what makes an HTTP check burn its whole timeout instead of failing fast. It was built, then removed: on this Windows host *every* closed local port times out rather than refusing, so it fired on all six ports at once. A check that warns about everything says nothing. Kept as a comment in `_port_in_use` so the next person doesn't rebuild it |
| `src/diagnostics/` and a parallel `cinemagraph/doctor.py` were written independently, and reconciled onto the former (2026-08-21) | the second was built without noticing the first, which sat unmerged on `feat/run-commands`. `src/diagnostics/` won on placement (the §5.1 argument above, which the duplicate got wrong by putting satellite and Docker knowledge inside the core package) and on coverage (ports, GPU headroom, bundle freshness, tooling). Three things were ported *from* the duplicate: detection of the busy watchdog's wedged-503 state, which postdated the original; an ffmpeg check, since it's the render path's one hard dependency and lives inside a wheel where `which` won't find it; and a library check that resolves its path instead of calling `library_root()`, which mkdirs — a diagnostic must not create the thing it reports on |
| A wedged satellite recovers by killing its own process (`BUSY_TIMEOUT` → `os._exit`), not by unloading in place (2026-08-21) | a hung CUDA call never raises into Python, its worker thread can't be cancelled, and `torch.cuda.empty_cache()` can't release memory the wedged interpreter still owns — so there is no in-process recovery to write. Worse, the idle TTL was *defeated* by this case: the reaper took the same lock the stuck request held, and blocked on it forever. Exiting is only a recovery when paired with `restart: unless-stopped`, so the two land together; the pattern is LocalAI's idle/busy watchdog pair. See `docs/experiments/2026-08-21-model-lifecycle-prior-art.md` |
| Satellite `/health` reports in-flight state, and 503s when wedged (2026-08-21) | a liveness check that only proves the event loop is alive cannot see this failure — `/health` never took the inference lock, so a wedged service answered a cheerful 200 forever while `/capabilities` kept advertising a capability that could no longer complete. Needed no core change: `service_available()` already keys on `status_code == 200`. An ordinary in-flight request deliberately still passes, or `/capabilities` would flap on every normal generation |
...
| `server/ui.py`'s seven-tab page ported into `frontend/` (2026-08-14) | the "revisit only if the UI outgrows a single page" trigger this document set for itself in §5.5 was met: seven tabs, three feature-gated, one asset picker reused six times, a Web Audio waveform editor, and per-tab polling plus a save-to-library flow. The scaffold's stack was already pinned (React 19 / Vite / TanStack Router + Query / Tailwind v4 / shadcn conventions / generated `@hey-api` client / Biome / Playwright) -- the port neither chose nor re-litigated it. Behaviour is 1:1; every call goes through the generated SDK, so the pre-commit codegen hook keeps the client honest against `src/server/`. `ui.py` is left in place and still served at `GET /` -- deleting it, and serving the built SPA in production, are separate calls |
| Every frontend route namespaced under `/ui`, with `/` redirecting there | `server.app:app` serves its routes unprefixed (no `/api` base), so it already owns top-level paths including `/library` and `/assemble`; a client-side route on either name is indistinguishable from the API path it shadows. Found by a Playwright test navigating to `/library` and getting JSON. Filtering the dev proxy on the `Accept` header would separate navigations from SDK calls, but would also break the Library tab's plain download links for non-previewable assets -- those are navigations to an API path, and would start serving the SPA instead of the file. Namespacing the app leaves the API's URL space untouched and needs no heuristics |
| `server/ui.py` deleted; `GET /` serves `frontend/dist` (2026-08-14) | the port above made it dead weight -- two UIs implementing the same seven tabs, one of them no longer the one being changed. Served through ordinary routes rather than `app.mount(StaticFiles(...))`: a mount resolves its directory at import time (the "config frozen at import" pattern `config.py` exists to escape) and can't be overridden in tests. The SPA fallback is scoped to `/ui`, never the root, so it can't shadow `/library`/`/assemble`/`/jobs`. A missing bundle returns a 503 page explaining how to build it while every other route keeps working -- the same degrade rule the optional services follow, applied to the fact that a bundle, unlike a Python string, can genuinely be absent |
| `frontend/package.json`'s `build` reordered to `vite build && tsc -b` | not cosmetic: the TanStack Router plugin generates `src/routeTree.gen.ts` *during* the Vite build, and that file is gitignored, so type-checking first fails on every clean checkout. `test-frontend.yml` and any Docker build were both already broken by this and would have stayed broken; found by deleting the generated file and running the build the way CI does |
| `Settings` gained `populate_by_name=True` (2026-08-14) | a real, silent, pre-existing bug found while adding `frontend_dist_dir`: setting `validation_alias` on a field makes it settable *only* by that alias, and `extra="ignore"` then swallows the field-name form without complaint. So `Settings(data_dir=tmp_path)` -- what every test fixture and `dependency_overrides` call in this repo uses -- did nothing, leaving `data_dir` at its default and pointing tests at the repo's own `./data`. Nothing asserted on the path, so the whole suite passed while ignoring its own isolation. Caught because the new frontend tests do assert on the path |
| Feature tabs are never hidden; availability is a status dot (2026-08-23) | hiding a tab had been doing double duty as the *explanation* for why a feature was unavailable, so the app's shape changed with whatever containers happened to be running and a feature you'd never enabled was indistinguishable from one that didn't exist. Making them all visible required `ConfigHint` to grow a second case in the same change (not-configured, as distinct from configured-but-down) -- otherwise a newly-visible tab renders a working-looking form that silently can't submit, which is a worse failure than the vanishing tab it replaced. See `docs/experiments/2026-08-23-ui-redesign.md` |
| Deployment facts (VRAM, pricing, what a satellite is *for*) live in the frontend's `lib/engines.ts`, not on `ServiceStatus` (2026-08-23) | they're properties of a deployment, not things the running API observes about itself -- an API that reported its own pricing would be repeating a constant back to the caller. The dividing line: if the API can *observe* it, ask the API. Which engines can remix is therefore read live from `music_remix_models` rather than duplicated as a static flag, so registering a third music adapter needs no frontend change at all |
| Job state lifted out of the submitting component into a `JobsProvider` above the router (2026-08-23) | not a refactor for tidiness -- a data-loss fix. `useJobRunner` kept the poll in the component that submitted, so navigating away unmounted it, cancelled the loop and dropped the job id, while the render carried on server-side with its file still downloadable and no route back to it. The hook's public shape was kept identical, so six route files and `JobResult` needed no edits. Deliberately in memory only, never localStorage: `server/jobs.py` is an in-process dict a restart clears, so persisted ids would resurrect confident-looking rows pointing at 404s -- both sides forget together. Guarded by a Playwright spec verified to fail against the previous implementation |
